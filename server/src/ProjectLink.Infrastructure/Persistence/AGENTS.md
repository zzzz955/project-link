...
| `StageEndTransactionRepository.cs` | `StageEndTransactionRepository` | Atomic stage-end TX (4 tables + daily increment) |
...
| symbol | kind | note |
|--------|------|------|
| `StageEndTransactionRepository.ExecuteAsync` | method | FOR UPDATE NOWAIT on stage_progress, stage_best_records; FOR UPDATE on user_currency, user_ranking_cache |
...
- Lock order in StageEndTransaction: stage_progress → stage_best_records → user_currency → user_ranking_cache (prevents deadlocks)
