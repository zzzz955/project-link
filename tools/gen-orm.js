...
async function getExistingTables(client, dbType) {
  ...
  if (dbType === 'sqlite') {
    const rows = client.prepare(`SELECT name FROM sqlite_master WHERE type='table'`).all();
    return new Set(rows.map(r => r.name));
  }
  return new Set();
}

async function getColumnTypes(client, tableName, dbType) {
  if (dbType === 'postgresql') {
    const res = await client.query(
  ...
  if (dbType === 'mysql') {
    ...
    return Object.fromEntries(rows.map(r => [r.column_name, r.column_type.toLowerCase()]));
  }
  return {};
}
...
async function main() {
  ...
  // Detect tables removed from schema → add DROP to SQL (never auto-execute)
  if (client) {
    const dbTables = await getExistingTables(client, dbType);
    for (const dbTable of dbTables) {
      if (!schemaTables.has(dbTable)) {
        const dropSQL = `DROP TABLE IF EXISTS ${dbTable};`;
        dropLines.push(dropSQL);
        console.log(`[gen-orm] NOTICE: Table "${dbTable}" not in schema — DROP TABLE added to SQL (manual review required).`);
      }
    }
  }
  ...
  for (const tbl of schema.tables) {
    ...
    if (!exists) {
      ...
      const existingCols   = client ? await getExistingColumns(client, tbl.name, dbType) : new Set();
      const existingColTypes = client ? await getColumnTypes(client, tbl.name, dbType) : {};
      ...
      for (const col of tbl.columns) {
        ...
        } else if (client && existingColTypes[col.name] !== undefined) {
          // Detect column type change → add ALTER TABLE MODIFY to SQL (manual review)
          const expectedSqlType = resolveType(col.type, dbType).toLowerCase();
          const actualType = existingColTypes[col.name];
          if (!actualType.startsWith(expectedSqlType.split('(')[0])) {
            const colSQL = buildColumnSQL(col, dbType);
            const modifySQL = dbType === 'mysql'
              ? `ALTER TABLE ${tbl.name} MODIFY COLUMN ${colSQL};`
              : `ALTER TABLE ${tbl.name} ALTER COLUMN ${col.name} TYPE ${resolveType(col.type, dbType)};`;
            alterLines.push(`-- TYPE CHANGE: ${tbl.name}.${col.name} (${actualType} -> ${expectedSqlType})`);
            alterLines.push(modifySQL);
            console.log(`[gen-orm] NOTICE: ${tbl.name}.${col.name} type changed (${actualType} -> ${expectedSqlType}) — ALTER added to SQL (manual review required).`);
          }
        }
      }
    }
  ...
  if (dropLines.length) {
    sqlLines.push('-- Tables removed from schema (review before applying):');
    sqlLines.push(...dropLines, '');
  }
